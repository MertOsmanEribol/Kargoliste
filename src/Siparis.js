      ...
      const fetchData = async () => {
        try {
          const response = await axios.get(
            "https://private-d90551-merteribol.apiary-mock.com/OrderList"
          );
                ...
                <div className="portlet-body flip-scroll">
                  <table className="table table-bordered table-striped table-condensed flip-content">
                    ...
                    {order.map((OrderList, index) => (
                          <tr key={index}>
                            <td>{OrderList.siparis_veren} </td>
							<td>{OrderList.urun_id}</td>
							<td>{OrderList.adet}</td>
							<td>{OrderList.birim_fiyat}</td>
                            <td>{OrderList.toplam_fiyat}</td>
							<td>{OrderList.para_birimi}</td>
                          </tr>
                        ))}
                    </tbody>
                  </table>
